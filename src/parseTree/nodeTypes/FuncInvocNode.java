...
public class FuncInvocNode extends ParseNode {
	private Binding binding;
	private Scope declarationScope;

	public FuncInvocNode(Token token) {
		super(token);
		assert(token instanceof IdentifierToken);
		this.binding = null;
	}
	public FuncInvocNode(ParseNode node) {
		super(node);

		if(node instanceof FuncInvocNode) {
			this.binding = ((FuncInvocNode) node).binding;
		} else {
			this.binding = null;
		}
...
// attributes
	...
	public Boolean isMutable() {
		return (binding != null) ? binding.isConstant() : null;
	}
...
// convenience factory
	...
	public static FuncInvocNode withChildren(Token token, ParseNode index) {
		FuncInvocNode node = new FuncInvocNode(token);
